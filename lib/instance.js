module.exports = function(settings) {
    ...
    var server = http.createServer(function (req, res) {
        ...
        try {
            ...
            if (sites.length > 0) {
                ...
                if (!router(req, res, siteRoutes, function(mode, req, res, serve) {
                    if (mode == 'beforeRequest' && sites[0].beforeRequest.length > 0) {
                        var a;
                        for (a = 0; a < ehs_instance.global.beforeRequest.length ; a++) { 
                            if(!sites[0].beforeRequest[a](req, res)) {
                                return false;
                            }
                        }

                        var b;
                        for (b = 0; b < sites[0].beforeRequest.length ; b++) { 
                            if(!sites[0].beforeRequest[b](req, res)) {
                                return false;
                            }
                        }
                    } else if (mode == 'beforeResponse' && sites[0].beforeRequest.length > 0) {
                        var a;
                        for (a = 0; a < sites[0].beforeResponse.length ; a++) { 
                            if(!sites[0].beforeRequest[a](req, res)) {
                                return false;
                            }
                        }
                        ...
                        var b;
                        for (b = 0; b < sites[0].beforeResponse.length ; b++) { 
                            if(!sites[0].beforeResponse[b](req, res)) {
                                return false;
                            }
                        }
                    } else if (mode == 'serve') {
            ...
            if (!router(req, res, globalRoutes, function(mode, req, res, serve) {
                if (mode == 'beforeRequest' && ehs_instance.global.beforeRequest.length > 0) {
                    var a;
                    for (a = 0; a < ehs_instance.global.beforeRequest.length ; a++) { 
                        if(!sites[0].beforeRequest[a](req, res)) {
                            return false;
                ...
                } else if (mode == 'beforeResponse' && ehs_instance.global.beforeResponse.length > 0) {
                    var a;
                    for (a = 0; a < ehs_instance.global.beforeResponse.length ; a++) { 
                        if(!sites[0].beforeRequest[a](req, res)) {
                            return false;
                        }
                    }
    ...
    return {
        reset:function() {
            ehs_instance.global.beforeRequest = [];
            ehs_instance.global.beforeResponse = [];
        ...
        beforeRequest:function(callback) {
            ehs_instance.global.beforeRequest.push(callback);
        },
        beforeResponse:function() {
            ehs_instance.global.beforeResponse.push(callback);
        },
        ...
        createSite:function(hosts) {
            ...
            return {
                ...
                beforeRequest:function(callback) {
                    site.beforeRequest.push(callback);
                },
                beforeResponse:function() {
                    site.beforeResponse.push(callback);
                },
